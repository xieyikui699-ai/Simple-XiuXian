...
调平只改引擎常量表（`ZHENYUAN_BASE` 10→130、悟性系数 0.1→0.5），公式与月结顺序未动。1200 局（三难度档）模拟：飞升通关中位 36–38 游戏年（锚点 30–60 ✅）、吞并/被吞并 0%（锚点 ≤25% ✅）、两宗等级差 ≤1 占比 0.94（锚点"多数时间" ✅）。详见 `docs/design/2026-09-06-小程序简化版设计.md` §数值调平 与 `scripts/balance-report.json`。
